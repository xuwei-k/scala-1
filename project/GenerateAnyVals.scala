...
import scala.language.implicitConversions"""
    ...
    def implicitCoercions: List[String] = {
      ...
      else coercionComment.lines.toArray.map(_.toString).toList ++ coercions
    }
    ...
    def objectLines = {
      ...
      interpolate(comp + allCompanions + "\n" + nonUnitCompanions).trim.lines.toArray.map(_.toString).toList ++ (implicitCoercions map interpolate)
    }
  ...
  sealed abstract class AnyValRep(val name: String, val repr: Option[String], val javaEquiv: String) {
    ...
    def indentN(s: String) = s.lines.toArray.map(_.toString).toList map indent mkString "\n"
...
override def getClass(): Class[Boolean] = ???
    """.trim.lines.toArray.map(_.toString).toList

    def objectLines = interpolate(allCompanions + "\n" + nonUnitCompanions).lines.toArray.map(_.toString).toList
  }
...
"""
    ...
    def objectLines = interpolate(allCompanions).lines.toArray.map(_.toString).toList

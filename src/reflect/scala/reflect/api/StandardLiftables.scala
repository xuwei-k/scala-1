...
trait StandardLiftables { self: Universe =>
  ...
  trait StandardLiftableInstances {
    ...
    implicit def liftLeft[L: Liftable]: Liftable[Left[L]]               = Liftable { case Left(v)  => callScala(stdnme.util, stdnme.Left)(lift(v) :: Nil) }
    implicit def liftRight[R: Liftable]: Liftable[Right[R]]             = Liftable { case Right(v) => callScala(stdnme.util, stdnme.Right)(lift(v) :: Nil) }
